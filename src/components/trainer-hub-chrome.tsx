...
/**
 * The trainer's own frame: header, tab row, role pill, course switcher, chat
 * bar and phone doors.
 *
 * MCT polish pass §10 (16 Sep 2026): "when a trainer opens anything that
 * belongs to a candidate, it opens in the trainer's own frame, full width,
 * with none of the trainee-side chrome". That means this chrome has to be
 * mountable outside the (hub) route group, because a candidate's pages live
 * under /portfolio and a Next layout cannot reach across route groups. So the
 * hub's layout is a one-line caller of this now, and the candidate shell's
 * staff branch is the second.
 *
 * Everything it reads is scoped to the VIEWER -- their profile, their course,
 * their centre, their chat -- so it computes the same thing wherever it is
 * mounted.
 */
export async function TrainerHubChrome({
  children,
  banner,
  bare = false,
  suppressChat = false,
  ...
  /** Rendered directly under the header: §10's "Viewing as tutor" strip. */
  banner?: React.ReactNode;
  /** The page brings its own frame (the candidate's pages do). */
  bare?: boolean;
  /** The page mounts its own chat (the candidate shell swaps it in preview). */
  ...
  return (
    ...
    <div className="flex min-h-full flex-1 flex-col" style={hubVars}>
      ...
      <header className="border-b border-border bg-frame">
        ...
        <div className="flex flex-wrap items-center gap-x-[18px] gap-y-2 px-[22px] py-2 sm:h-14 sm:flex-nowrap sm:py-0">
          ...
          {/* Wraps inside itself below sm, and only below sm. The row above
              ...
              joined it afterwards, so a shrink-0 cluster pushed the document
              to 470px on a 375px screen and to 825px on a 768px one, and the
              hub scrolled sideways at both (walked 23 Sep 2026). It needs
              528px beside a ~297px wordmark and tab row, so it stays wrappable
              until lg rather than sm: a tablet has no more room for it than a
              phone does. From lg up it is the single unshrinkable row it
              always was. */}
          <div className="flex min-w-0 flex-wrap items-center gap-[11px] lg:shrink-0 lg:flex-nowrap">
      ...
      <div className={`container flex-1 pt-8 ${profile && staffChat && !suppressChat ? "pb-28" : "pb-8"} ${phoneBar ? "max-md:pb-40" : ""}`}>
        {bare ? (
      ...
      {profile && staffChat && !suppressChat ? (
        <StaffChatDrawer
