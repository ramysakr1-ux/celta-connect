...
export function UndoImportButton({ importId }: { importId: string }) {
  ...
  if (state.removed) {
    return <span className="text-label text-muted">Removed {state.removed}</span>;
  }
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        disabled={pending}
        className="wash rounded-[6px] border border-border bg-card-inset px-3 py-1.5 text-label font-medium text-ink hover:border-destructive hover:text-destructive disabled:opacity-60"
      >
        {pending ? "Undoing..." : "Undo this import"}
      </button>
      {state.error ? <p className="max-w-xs text-right text-label text-destructive">{state.error}</p> : null}
    </form>
  );
}
