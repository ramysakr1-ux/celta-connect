...
export function CoursebookList({
  ...
  return (
    <div className="flex flex-col gap-5">
      {levels.map((level) => (
        <div key={level}>
          <h3 className="font-serif text-label font-bold tracking-[0.12em] text-muted uppercase">{level}</h3>
            ...
            {byLevel
              ...
              .map((cb) => (
                ...
                >
                  ...
                  <span className="text-body text-muted">{STATUS_LABEL[cb.generation_status]}</span>
                </Link>
              ))}
          </div>
        </div>
