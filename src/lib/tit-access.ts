...
// cache()'d on primitives: the hub layout (for the tab) and the TinT page
// (for the records) ask this for the same viewer in the same request.
const access = cache(
  async (
    ...
    const visible = all.filter((t) => t.profile_id === profileId || t.supervisor_profile_id === profileId || grantedIds.has(t.id));
