...
// per centre so it can be seen, and leave it at that."
//
// Two separate limits, and they come from two separate sections of the
// Administration Handbook June 2025 -- this file and the screens reading it
// cited 13.3 for both (walked 15 Sep 2026):
//
//   13.3 (p37): "the same assessor must not be used for more than two
//   consecutive courses, regardless of whether they are remote or
//   face-to-face" -- a run through the centre's own course sequence.
//
//   13.8 (p38): "An assessor must not assess more than two concurrent
//   courses at a centre" -- date-range overlap.
//
// More than two is the breach in both, so two is the limit, not past it.
// Visible-only, never enforced: this computes the two figures, it doesn't
// gate anything.
...
export interface AssessorHistoryEntry {
   ...
   *  the figure Handbook 13.8's "two concurrent" cap is about. */
  ...
  /** Only true when either figure has reached or passed its Handbook limit
   *  (13.3 consecutive, 13.8 concurrent) -- purely informational, per the
   *  design spec ("never blocks"). */
  flag: boolean;
  flagText: string;
}
...
// consecutive-streak and peak-concurrent figures since 13.3 and 13.8 set
// the same cap (two) on each.
...
export function computeAssessorCentreHistory(
  ...
  for (const [profileId, courseIdSet] of courseIdsByProfile) {
    ...
    if (peakConcurrentSeverity === "over") {
      reasons.push(`Assessed ${numberWord(peakConcurrent)} courses concurrently — over the two-course concurrent limit in Handbook 13.8.`);
    } else if (peakConcurrentSeverity === "at-limit") {
      reasons.push("Assessed two courses concurrently — at the two-course concurrent limit in Handbook 13.8.");
    }
