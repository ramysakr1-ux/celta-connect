...
export async function getActivePaymentProvider(centerId: string): Promise<PaymentProviderAdapter> {
  ...
  // it and never finished onboarding -- so refusing is more honest than
  // sending an applicant to a checkout that cannot complete.
  const chosen = data?.payment_provider_connected_at ? data.payment_provider : null;
  ...
  switch (chosen) {
    ...
    // Every other provider in providers.ts lands here until its adapter
    // exists. Stripe holds no privileged position -- it is simply the one
    // that was written first, and could be removed without affecting
