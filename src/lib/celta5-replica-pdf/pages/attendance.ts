...
export interface AbsenceRow {
  ...
  tutorComment: string | null; // "other absences" sub-table only
}
...
//
// attendance_absences has no "session missed" column for either
// sub-table, and no "candidate comment" column for the "other" sub-table
// -- both real form columns, neither tracked anywhere in Connect. Left
// blank, same as the observations page's untracked signature column.
...
export function drawAttendancePage(page: PDFPage, fonts: Celta5Fonts, data: AttendancePageData) {
  ...
  data.unavoidableAbsences.slice(0, UNAVOIDABLE_ROW_DIVIDERS.length - 1).forEach((row, i) => {
    drawTableRow(page, fonts.regular, [row.date, "", row.reason ?? "", row.workMadeUp ?? ""], UNAVOIDABLE_COLUMNS, UNAVOIDABLE_ROW_DIVIDERS, i, 7);
  });

  data.otherAbsences.slice(0, OTHER_ROW_DIVIDERS.length - 1).forEach((row, i) => {
    drawTableRow(page, fonts.regular, [row.date, "", row.reason ?? "", row.workMadeUp ?? "", row.tutorComment ?? ""], OTHER_COLUMNS, OTHER_ROW_DIVIDERS, i, 7);
  });
}
