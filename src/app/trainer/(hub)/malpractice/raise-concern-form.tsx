  ...
  return (
    <form className="sheet flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-label text-muted">Most concerns turn out to be nothing -- save a note if you're not sure, or open a case if you are.</p>
      </div>

      <div className="flex flex-col gap-2">
        <p className="text-body font-medium text-ink">What kind of concern is this?</p>
        {KINDS.map((k) => (
          <button
            key={k.value}
            type="button"
          ...
          >
            ...
            <span className="text-label text-muted">{k.hint}</span>
          </button>
        ))}
      </div>

      <div className="flex flex-col gap-1.5">
        <label className="text-body font-medium text-ink">What have you found?</label>
        <textarea
          name="findings"
          rows={4}
      ...
      <div className="flex flex-col gap-1.5 rounded-[6px] border border-border-faint bg-surface-muted/40 p-3">
        ...
        <p className="text-label text-ink">{ownSubmissionLabel}</p>
        <p className="text-label text-ink">AI declaration: {aiDeclared ? "used" : "not used"}</p>
      </div>

      {noteState.error ? <p className="text-body text-destructive">{noteState.error}</p> : null}
      {caseState.error ? <p className="text-body text-destructive">{caseState.error}</p> : null}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          ...
          className="rounded-[6px] border border-border px-4 py-2 text-body font-medium text-ink disabled:opacity-60"
        >
        ...
        <button
          ...
          disabled={notePending || casePending || !kind}
          className="rounded-[6px] bg-destructive px-4 py-2 text-body font-medium text-card disabled:opacity-60"
        >
