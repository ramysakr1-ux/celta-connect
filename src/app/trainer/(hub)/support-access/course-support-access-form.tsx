...
export function CourseSupportAccessForm({ courseName, grants }: { courseName: string; grants: SupportGrantRow[] }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <h1 className="font-serif text-h1 text-ink">Platform support access</h1>
      ...
      <div className="sheet">
        <h2 className="font-serif text-h3 text-ink">Grant access to {courseName}</h2>
        <p className="mt-1 text-body text-muted">Grades, marking, and this course&apos;s timetable. Not course chat, unless you add it.</p>
        <form action={action} className="mt-4 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select id="duration_hours" name="duration_hours" required className="h-9 w-40 rounded-[6px] border border-input bg-card px-2 text-body text-ink">
      ...
      <div>
        <h2 className="font-serif text-h3 text-ink">Access log for {courseName}</h2>
        <div className="sheet mt-3 overflow-hidden !p-0">
          {grants.length === 0 ? (
            <p className="p-6 text-body text-muted">No grants have ever been made for this course.</p>
          ) : (
            <ul>
              {grants.map((g) => (
