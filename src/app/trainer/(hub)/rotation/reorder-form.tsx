...
import { reorderSubgroup, type FormState } from "@/app/trainer/(hub)/rotation/actions";
    ...
    <form action={action} className="flex flex-col gap-3">
      ...
      {ordered.map((m) => (
        <div key={m.traineeId} className="flex items-center gap-3">
          ...
          <label className="text-body text-muted" htmlFor={`position__${m.traineeId}`}>
            Position
          </label>
          <select
            ...
            className="appearance-none rounded-[6px] border border-border bg-card px-2 py-1 text-center text-body text-ink outline-none focus:border-primary"
          >
            {ordered.map((_, i) => (
