...
export default async function ConcernsInboxPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="sheet">
        <p className="text-label text-muted">{(concerns ?? []).length} total</p>
      ...
      {(concerns ?? []).length === 0 ? (
        <p className="sheet text-body text-muted">No concerns raised yet.</p>
      ) : (
            ...
            <div key={c.id} className="sheet flex flex-col gap-2">
              <div className="flex items-baseline justify-between gap-3">
                <p className="text-body font-semibold text-ink">
                  ...
                  <span className="ml-2 text-label font-normal text-muted">Routed to {ROUTE_LABEL[c.route] ?? c.route}</span>
                </p>
                <p className="text-label text-muted">{formatDate(c.created_at, timeZone, { year: "numeric" })}</p>
              </div>
              <p className="text-body whitespace-pre-wrap text-ink">{c.body}</p>
              ...
              {c.response ? (
                <div className="mt-1 rounded-[6px] bg-surface-muted/40 p-3">
                  ...
                  <p className="mt-1 text-body text-ink">{c.response}</p>
                </div>
              ) : (
