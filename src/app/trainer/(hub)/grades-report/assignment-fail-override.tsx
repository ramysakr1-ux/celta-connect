...
export function AssignmentFailOverride({
  ...
  if (overrideReason) {
    return (
      <div className="flex flex-col gap-1.5 rounded-[6px] border border-border-faint bg-surface-muted/40 px-3 py-2">
        <p className="text-label font-bold tracking-[0.1em] text-muted uppercase">Ceiling overridden</p>
        <p className="text-label leading-relaxed text-ink">{overrideReason}</p>
        {overriddenByName ? <p className="text-label text-muted">Recorded by {overriddenByName}</p> : null}
        <form action={action} className="self-start">
          <input type="hidden" name="trainee_id" value={traineeId} />
          <input type="hidden" name="clear" value="1" />
          <button type="submit" disabled={pending} className="text-label text-primary hover:underline disabled:opacity-60">
  ...
  if (!open) {
    return (
      <button type="button" onClick={() => setOpen(true)} className="self-start text-label text-muted hover:text-primary hover:underline">
  ...
  return (
    <form action={action} className="flex flex-col gap-1.5 rounded-[6px] border border-border-faint px-3 py-2">
      <input type="hidden" name="trainee_id" value={traineeId} />
      <label htmlFor={`override-${traineeId}`} className="text-label font-bold tracking-[0.1em] text-muted uppercase">
      ...
      <textarea
        ...
        className="rounded-[6px] border border-border bg-card-inset px-2.5 py-1.5 text-meta text-ink outline-none focus:border-primary"
      />
      <div className="flex items-center gap-3">
        ...
        <button type="button" onClick={() => setOpen(false)} className="text-label text-muted hover:underline">
          Cancel
        </button>
      </div>
      {state.error ? <p className="text-label text-destructive">{state.error}</p> : null}
