  ...
  return (
    <>
      <div className="sheet flex flex-col gap-3 p-6">
        <h2 className="font-serif text-h3 text-ink">{editing ? `Edit "${editing.term}"` : "Add a term"}</h2>
        <p className="text-body text-muted">
          One word or phrase as a tutor would actually type it, and the codes it should attach. Separate codes with
          spaces or commas.
        </p>
        <form action={action} className="flex flex-wrap items-end gap-3">
          <label className="flex min-w-[220px] flex-1 flex-col gap-1.5 text-body text-muted">
            Word or phrase
            <input
              ...
              className="h-9 rounded-[6px] border border-border bg-card-inset px-3 text-body text-ink outline-none focus:border-primary read-only:text-muted"
            />
          </label>
          <label className="flex min-w-[180px] flex-1 flex-col gap-1.5 text-body text-muted">
            Criteria codes
            <input
          ...
          <button
            type="submit"
            disabled={pending}
            className="h-9 rounded-[6px] bg-primary px-4 text-body font-medium text-card disabled:opacity-60"
          >
          ...
          {editing ? (
            <button
              ...
              className="h-9 rounded-[6px] border border-border px-4 text-body text-ink"
            >
              Cancel
            </button>
          ) : null}
      ...
      <div className="sheet p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="font-serif text-h3 text-ink">Every term</h2>
          <input
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Find a word or a code"
            className="h-9 w-56 rounded-[6px] border border-border bg-card-inset px-3 text-body text-ink outline-none focus:border-primary"
          />
        </div>
...
function Section({
  ...
  return (
    <div className="mt-5">
      ...
      <p className="mt-0.5 text-label text-muted">{blurb}</p>
      <ul className="mt-2 flex flex-col">
        {terms.length === 0 ? (
          <li className="py-3 text-body text-muted">Nothing matches.</li>
        ) : (
          terms.map((t) => (
            <li
              key={t.term}
            ...
            >
              <span className={`min-w-[150px] text-body font-medium ${t.enabled ? "text-ink" : "text-muted line-through"}`}>
                {t.term}
              </span>
              ...
              {t.changed && t.editorName ? (
                <span className="text-label text-muted">changed by {t.editorName}</span>
              ) : null}
              <span className="ml-auto flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => onEdit(t)}
                  className="rounded-[6px] border border-border px-2.5 py-1 text-label text-ink hover:border-primary"
                >
                  Edit
                </button>
                <form action={setGlossaryTermEnabled}>
                  ...
                  <button type="submit" className="rounded-[6px] border border-border px-2.5 py-1 text-label text-ink hover:border-primary">
                ...
                {!t.builtIn ? (
                  <form action={removeGlossaryTerm}>
                    <input type="hidden" name="term" value={t.term} />
                    <button type="submit" className="rounded-[6px] border border-border px-2.5 py-1 text-label text-destructive hover:border-destructive">
                      Remove
                    </button>
                  </form>
                ) : null}
