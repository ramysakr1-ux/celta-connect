...
// Trainee-facing view of TP1-6: every slot is always visible (so a trainee
...
// shows the outcome underneath the plan. Read-only throughout: trainees have
// no insert/update/delete policy on plan_assignments or tp_lessons.
          ...
          <Link key={tpNumber} href={`/dashboard/trainee/plan/${tpNumber}`} className="card-interactive block p-6">
            ...
            <div className="mt-4 flex flex-col gap-3">
              <div>
                ...
                <p className="text-ink">{plan.main_lesson_aim}</p>
              </div>
              {plan.sub_aim ? (
                <div>
                  <p className="text-sm text-muted">Sub aim</p>
                  <p className="text-ink">{plan.sub_aim}</p>
                </div>
              ) : null}
              {plan.materials_description ? (
                <div>
                  <p className="text-sm text-muted">Materials</p>
                  <p className="text-ink">{plan.materials_description}</p>
                </div>
              ) : null}
              {plan.page_references ? (
                <div>
                  <p className="text-sm text-muted">Page references</p>
                  <p className="text-ink">{plan.page_references}</p>
                </div>
              ) : null}
            </div>

            {tier !== "minimal" && plan.procedure ? (
              <div className="mt-4 border-t border-border-faint pt-4">
                <p className="text-sm text-muted">Procedure</p>
                <div className="mt-2">
                  <ProcedureDisplay tier={tier} procedure={plan.procedure} />
                </div>
              </div>
            ) : null}

            {showsTips(tier) ? (
              <div className="mt-4 grid grid-cols-1 gap-4 border-t border-border-faint pt-4 sm:grid-cols-2">
                <div>
                  <p className="text-sm text-muted">Before you teach</p>
                ...
                <div>
                  <p className="text-sm text-muted">In the lesson</p>
                  <ul className="mt-1 list-disc space-y-1 pl-5 text-sm text-muted">
                    {TEACHING_TIPS.map((tip, i) => (
                      <li key={i}>{tip}</li>
                    ))}
                  </ul>
                </div>
            ...
            {showsAbbreviationsGlossary(tier) ? (
              <details className="mt-4 border-t border-border-faint pt-4">
                <summary className="cursor-pointer text-sm text-muted hover:text-ink">
                  Abbreviations
                </summary>
                <dl className="mt-2 grid grid-cols-2 gap-x-6 gap-y-1 sm:grid-cols-3">
                  {ABBREVIATIONS_GLOSSARY.map(({ abbr, meaning }) => (
                    <div key={abbr} className="flex gap-2 text-sm">
                      <dt className="font-medium text-ink">{abbr}</dt>
            ...
            {lesson ? (
              <div className="mt-4 border-t border-border-faint pt-4">
                ...
                {lesson.tutor_comments ? (
                  <p className="mt-2 text-ink">{lesson.tutor_comments}</p>
                ) : null}
              </div>
            ) : null}
