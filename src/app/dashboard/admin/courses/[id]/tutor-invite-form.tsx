...
export function TutorInviteForm({ courseId }: { courseId: string }) {
  ...
  return (
    <form action={action} className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_auto_auto]">
      ...
      <button type="submit" disabled={pending} className="wash h-9 rounded-[6px] border border-border px-3 text-sm text-ink hover:border-primary disabled:opacity-60">
