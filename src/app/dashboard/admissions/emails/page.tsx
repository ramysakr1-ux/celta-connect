      ...
      {(bounceTasks ?? []).length > 0 ? (
        <div className="flex flex-col gap-2">
          {(bounceTasks ?? []).map((b) => (
            <div
              key={b.id}
              className="flex items-start gap-3 rounded-[8px] border border-destructive/25 bg-destructive/5 px-4 py-3.5 ring"
            >
        ...
        {(emails ?? []).length === 0 ? (
          ...
          (emails ?? []).map((email) => {
            ...
            return (
              <div
                key={email.id}
                className="grid grid-cols-[1.3fr_1.1fr_1fr_1fr_1fr] items-center border-b border-border-faint px-4 py-3 last:border-none ring"
              >
