    ...
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-4 border-t border-border-faint pt-3">
          <Link href="/dashboard/admissions/emails" className="text-body font-medium text-primary hover:underline">
            Email delivery
          </Link>
          <Link href="/dashboard/admin/email-preview" className="text-body text-muted hover:text-ink">
      ...
      <div className="card flex flex-col gap-4 p-5">
        <h2 className="font-serif text-h3 font-semibold text-ink">AI reading of the selection task</h2>
        ...
        <div className="flex items-center justify-between gap-3 rounded-[6px] border border-border p-3">
          <div>
            <p className="text-body text-ink">Shadow mode</p>
            <p className="text-label text-muted">Records a reading on every applicant. Nothing is ever sent from this alone.</p>
          </div>
          <form action={toggleAdmissionsAiShadowMode}>
            ...
            <button
              ...
              className="rounded-full border border-border px-3 py-1 text-label font-semibold text-ink hover:border-primary disabled:opacity-50 wash"
            >
        ...
        <div className="flex items-center justify-between gap-3 rounded-[6px] border border-border p-3">
          <div>
            <p className="text-body text-ink">Auto-book the clear lane</p>
      ...
      <div className="card flex flex-col gap-4 p-5">
        <h2 className="font-serif text-h3 font-semibold text-ink">Extended writing task prompts</h2>
        <p className="text-body text-muted">
        ...
        <ul className="flex flex-col gap-2">
          {(prompts ?? []).map((p) => (
            <li key={p.id} className="flex items-center justify-between gap-3 rounded-[6px] border border-border p-3 hover-ring">
              <div>
                <span className="text-label font-semibold uppercase tracking-[0.06em] text-muted">{p.prompt_type}</span>
                <p className="text-body text-ink">{p.prompt_text}</p>
              </div>
              <form action={togglePromptActive}>
                <input type="hidden" name="prompt_id" value={p.id} />
                <input type="hidden" name="active" value={String(p.active)} />
                <button type="submit" className="text-label text-muted hover:text-ink">
        ...
        <form action={addWritingPrompt} className="flex flex-wrap items-end gap-3 border-t border-border pt-4">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="prompt_type" className="text-label text-muted">
              Type
            </label>
            <select id="prompt_type" name="prompt_type" required className="h-9 rounded-[6px] border border-input bg-card-inset px-2 text-body text-ink">
          ...
          <div className="flex flex-1 flex-col gap-1.5">
            <label htmlFor="prompt_text" className="text-label text-muted">
              Prompt
            </label>
            <input id="prompt_text" name="prompt_text" type="text" required className="h-9 rounded-[6px] border border-input bg-card-inset px-2 text-body text-ink" />
          </div>
      ...
      <div className="card flex flex-col gap-4 p-5">
        <h2 className="font-serif text-h3 font-semibold text-ink">Speaking task prompts</h2>
        <p className="text-body text-muted">
        ...
        <form action={addSpeakingPrompt} className="flex flex-wrap items-end gap-3 border-t border-border pt-4">
          <div className="flex flex-1 flex-col gap-1.5">
            <label htmlFor="speaking_prompt_text" className="text-label text-muted">
              Prompt
            </label>
            <input
              ...
              type="text"
              required
              className="h-9 rounded-[6px] border border-input bg-card-inset px-2 text-body text-ink"
            />
          </div>
          <button type="submit" className="rounded-[6px] bg-primary px-3 py-1.5 text-label font-semibold text-card">
            Add prompt
      ...
      <div className="card flex flex-col gap-4 p-5">
        <h2 className="font-serif text-h3 font-semibold text-ink">Interview question bank</h2>
        <p className="text-body text-muted">
          Seven fixed questions is the usual shape. Two more are drawn per applicant from the weak areas their task
          reading flagged.
        </p>
        {missingAreas.length > 0 ? (
          <div className="rounded-[6px] border border-border bg-surface-muted p-3 text-body text-ink">
        ...
        <ul className="flex flex-col gap-2">
          {(questions ?? []).map((q) => (
            <li key={q.id} className="flex items-center justify-between gap-3 rounded-[6px] border border-border p-3 hover-ring">
              <div>
                <span className="text-label font-semibold uppercase tracking-[0.06em] text-muted">{COVERAGE_LABEL[q.coverage_area]}</span>
        ...
        <form action={addInterviewQuestion} className="flex flex-wrap items-end gap-3 border-t border-border pt-4">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="coverage_area" className="text-label text-muted">
              Assesses
            </label>
            <select id="coverage_area" name="coverage_area" required className="h-9 rounded-[6px] border border-input bg-card-inset px-2 text-body text-ink">
          ...
          <div className="flex flex-1 flex-col gap-1.5">
            <label htmlFor="question_text" className="text-label text-muted">
              Question
            </label>
            <input id="question_text" name="question_text" type="text" required className="h-9 rounded-[6px] border border-input bg-card-inset px-2 text-body text-ink" />
          </div>
          <button type="submit" className="rounded-[6px] bg-primary px-3 py-1.5 text-label font-semibold text-card">
            Add question
          </button>
