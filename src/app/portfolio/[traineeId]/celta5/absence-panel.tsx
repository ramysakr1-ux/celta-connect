    ...
    <div className={booklet ? "flex flex-col gap-3" : "plain-card flex flex-col gap-3"}>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className={booklet ? "text-micro text-muted" : "mt-0.5 text-label text-muted"}>
            {booklet
      ...
      {!booklet && absences.length > 0 ? (
        <ul className="flex flex-col divide-y divide-border-faint">
          {absences.map((a) => (
            <li key={a.id} className="flex flex-col gap-0.5 py-2.5 first:pt-0">
              ...
              {a.reason ? <p className="text-label text-muted">{a.reason}</p> : null}
              {a.work_made_up ? <p className="text-label text-muted">Made up: {a.work_made_up}</p> : null}
              ...
              {a.tutor_comment ? (
                <p className="mt-1 rounded-[6px] border border-border-faint bg-accent/20 px-2.5 py-1.5 text-label text-ink">
        ...
        <p className="text-body text-muted">No absences recorded.</p>
      )}
      ...
      {open ? (
        <form action={formAction} className="flex flex-col gap-2 border-t border-border-faint pt-3">
          <label className="flex flex-col gap-1 text-label font-semibold text-ink">
            ...
            <input type="date" name="session_date" required className="rounded-[6px] border border-border bg-card-inset px-2.5 py-1.5 text-body font-normal text-ink outline-none focus:border-primary" />
          </label>
          <label className="flex flex-col gap-1 text-label font-semibold text-ink">
            Was it unavoidable?
            <select name="category" required defaultValue="unavoidable" className="rounded-[6px] border border-border bg-card-inset px-2.5 py-1.5 text-body font-normal text-ink outline-none focus:border-primary">
          ...
          <label className="flex flex-col gap-1 text-label font-semibold text-ink">
            What happened?
            <textarea name="reason" required rows={2} placeholder="A short line is enough." className="rounded-[6px] border border-border bg-card-inset px-2.5 py-1.5 text-body font-normal text-ink outline-none focus:border-primary" />
          </label>
          <label className="flex flex-col gap-1 text-label font-semibold text-ink">
            Have you made the work up? <span className="font-normal text-muted">(optional)</span>
            <input type="text" name="work_made_up" placeholder="e.g. watched the recording, read the input session" className="rounded-[6px] border border-border bg-card-inset px-2.5 py-1.5 text-body font-normal text-ink outline-none focus:border-primary" />
          </label>
          ...
          <p className="text-label text-muted">Once recorded this cannot be edited here — tell your tutor if you need it changed.</p>
        </form>
      ) : null}
    </div>
  );
