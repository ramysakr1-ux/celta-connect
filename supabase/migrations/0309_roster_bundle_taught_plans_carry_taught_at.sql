-- The roster bundle's taught plans carry their date.
--
-- The roster now counts TPs taught and passed AS OF today (tutor walk,
-- 20 Sep 2026): the demo's record clock writes the whole course at once, so
-- day 15 read "TPs passed 8 of 8". roster.ts checks plan_assignments.taught_at
-- for that -- and the bundle (0273, widened once already in 0286) selected
-- trainee_id and tp_number only, so on the bundled path every plan read as
-- undated, nothing counted, and assessed hours fell to 0.00. Same trap as
-- 0286's: a bundle is a second definition of a query and has to move with
-- the first. One more column; nothing else changes.
--
-- Re-runnable: create or replace, same signature, same grants.
...
as $$
  ...
  crs as (select total_hours, center_id, start_date, end_date from courses where id = p_course_id)
  select jsonb_build_object(
    ...
    'taught_plans', (select coalesce(jsonb_agg(to_jsonb(x)), '[]'::jsonb) from (select trainee_id, tp_number, taught_at from plan_assignments where course_id = p_course_id and taught_at is not null) x),
...
select count(*) as taught_plans_carry_taught_at
from jsonb_array_elements(public.hub_roster_bundle((select id from courses order by created_at desc limit 1)) -> 'taught_plans') x
where x ? 'taught_at';
