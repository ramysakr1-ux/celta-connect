-- specs/build-spec.md §3 "TP: the slot empties; nobody else moves." The UI
-- guard alone (hiding a withdrawn trainee's own AssignButton) isn't enough:
-- assign_tp_round is a whole-round bulk RPC keyed off subgroup_id+tp_number,
-- so any OTHER unassigned member's visible button still triggers it for
-- everyone in the round, including a withdrawn trainee, unless the function
-- itself skips them. v_subgroup_size (the rotation modulo base) stays the
-- real member count either way -- this only skips the INSERT for a
-- withdrawn member, it never changes v_subgroup_size or v_position for
-- anyone else, so nobody else's computed position moves.
...
begin
  ...
  loop
    ...
    -- §3 -- their slot is simply left empty; skip the insert, not the loop's
    -- effect on v_subgroup_size or anyone else's v_position.
    continue when exists (
