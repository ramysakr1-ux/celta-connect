-- Migration 0067 skipped a rotation assignment for anyone with
-- course_status <> 'active'. That was correct for 'withdrawn' but is wrong
-- for 'extension' (added in 0068) -- an extension candidate is still
-- actively teaching, just expected to finish after the official end date,
-- so their TP slot must NOT be skipped. Narrow the skip to the genuinely
-- frozen statuses only.
...
begin
  ...
  loop
    ...
    -- §3 -- only the genuinely frozen statuses leave the slot empty.
    -- 'extension' stays fully in rotation.
    continue when exists (
      select 1 from public.profiles
      where id = v_member.trainee_id and course_status in ('withdrawn', 'deferred', 'restarting')
    );
