-- Ramy, 28 Aug 2026. Fixes a real data bug found while building the answer
-- boxes: every task prompt had the FOLLOWING reading passage glued onto the
-- end of it. Migration 0230 extracted the UCLES 2018 PDF by splitting on
-- "Task N" markers, so everything between one task question and the next
-- task marker -- i.e. the next topic reading -- landed in the previous
-- task prompt. Task 1 ended with the lead-in for Task 2, Task 7 with a
-- paragraph about implicit grammar knowledge, and so on. Harmless while the
-- page was a read-only wall of text; wrong the moment each task carries its
-- own answer box, since the candidate is answering a question whose visible
-- text is mostly the next topic.
--
-- Re-extracted from the original PDF using the one boundary the document
-- itself marks: every task sits inside a pale-green filled box, so "inside
-- a box" is the question and "outside" is the reading. All 50 tasks
-- resolved cleanly that way against the source.
--
-- prompt keeps ONLY the question. lead_in is the reading that precedes it,
-- shown above the task as context rather than as part of what is asked.
alter table public.pre_course_task_items add column if not exists lead_in text;

update public.pre_course_task_items set prompt = '1. In what context will you be doing the CELTA course?

2. Do you know what context you will be teaching in after you finish the course?', lead_in = 'Section 1: Learners and Teachers, and the
...
Closed
The students usually all come from the same institution and no other learners join their group
...
planning.' where task_number = 1 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = '1. Think about why you decided to teach adults.
...
take into account the characteristics of adult learners.' where task_number = 2 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = '1. What would you want to find out about a group of learners that you had to teach so that you could plan
your lessons?

2. How would you find out?', lead_in = 'Finding out about learners
...
as people and learners.' where task_number = 3 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'If you were teaching a group of learners, each of whom had different motivations for learning English, which
learners would be the most challenging in terms of motivation?', lead_in = 'C
Motivation
...
Press pp. 22-23' where task_number = 4 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Look at the list of qualities and skills that a teacher might have.  Which do you think learners most often rate in
the top five?
...
•
is sensitive to the culture and backgrounds of the learners', lead_in = 'D
...
skills to enable them to achieve their goals.' where task_number = 5 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Make a list of associations you have with the word ‘grammar’.  (They may not all be negative!)', lead_in = 'Adapted from Scrivener, J (1994) Learning Teaching, Harlow: Macmillan Heinemann pp. 7-8
...
experiences in English language or second language classes when we were at primary or high school.' where task_number = 6 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Look at the following sentences and decide which are correct.  Write a correct version of the examples that are
incorrect.
...
6. People with 12 items or less can queue here.', lead_in = 'Grammar is sometimes perceived as being something abstract and difficult, associated with the analysis of very
...
However, native speakers do have implicit knowledge of grammar and use it correctly.' where task_number = 7 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Provide a list of reasons why English language teachers need to know about grammar.  In doing so, try to give
some thought to the learners’ perspective.', lead_in = 'A clear indication of our implicit grammar knowledge is our ability both to distinguish between correct and
...
our knowledge of grammar.' where task_number = 8 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Identify the underlined words in the following dialogue.  Use a grammar reference book or a dictionary to help
...
A: Oh, you mean(9) a plunger(10).', lead_in = 'Word class
...
information.' where task_number = 9 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Decide if the underlined verbs in the following sentences and questions are lexical verbs or auxiliary verbs.
1. He watches TV for at least two hours every evening.
2. What are you looking for?
3. They aren’t going to come.
4. What does he want?
5. They haven’t been here before.
6. He was waiting on the corner.', lead_in = 'Types of verbs
...
the word order i.e. Does he live here?' where task_number = 10 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Decide if the underlined verbs be, do and have have an auxiliary or lexical function in the following sentences
and questions.
...
8. Are they still here?', lead_in = 'There are three auxiliary verbs that have the function of creating different forms: be, do and have.  (Remember
...
the verb do (again in its past form did) functions as a lexical verb that carries meaning.' where task_number = 11 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'All of the following sentences (1-5) contain modal auxiliary verbs that are underlined.  Match the meaning of the
...
C. Advice
D. Possibility
E. Permission', lead_in = 'When be, do and have are used as auxiliaries, they do not really have any meaning as such.  However, there is
...
necessity.' where task_number = 12 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Match the underlined verb form examples to the correct definition.
...
simple tense.' where task_number = 13 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Think of the past form and the past participle forms of the following verbs.  Which are
...
go
drink
arrive', lead_in = 'Most of these forms have more than one use.  For example, -ing forms can be present participles when they act
...
verbs.' where task_number = 14 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Identify the different elements (past, present, modal, perfect, progressive, passive) underlined in the
...
5. I saw it coming.
6. This time next week you will be lying on a beach.', lead_in = 'Verb phrases
...
with the past participle of be been together with the -ing form of a lexical verb in order to create a verb
...
They give … vs. They gave …
2. Modal

They can / must / would / should give …
3. Simple
...
was given this brochure the past tense combines with the passive voice to create the past passive.' where task_number = 15 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Identify the tense names of the underlined verbs in the following sentences.  You will most likely need to
...
8. I have been feeling unwell for a while.', lead_in = 'While we have focused on these different elements in terms of identifying their form, it is worth remembering
...
combinations create the present perfect passive tense.' where task_number = 16 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'The following examples of language contain different tenses which are underlined.  The name of the tense
...
5. They own most of the land around here.  (present simple)', lead_in = 'Grammar and meaning
...
from reality rather than distance from the present.' where task_number = 17 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = '•
What auxiliary verb is used to create the present progressive?
•
What is the form of the lexical verb of the present progressive?', lead_in = 'Present progressive

In order to explore grammatical meaning in more detail, we will look at one example: the present progressive
tense.' where task_number = 18 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Look at the following mini-dialogues below and decide the time reference or use of the underlined
...
the street all on my own and I can hear the sound of footsteps …', lead_in = 'The following dialogue contains examples of the present progressive.  (N.B. the numbers at the beginning of
...
duration.  It is not expected they will continue forever.' where task_number = 19 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'What is the problem with the following examples?

1. He’s having a brother and a sister.
2. I’m liking this ice cream.
3. What are you thinking of your new job?
4. This sauce is lacking salt.', lead_in = 'In order to determine the use or meaning of tenses, it helps to look at them in context, even if it is a question of
...
speaking.' where task_number = 20 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Make a list of what extra information dictionaries can provide about words apart from the meaning.  It
would be a good idea to refer to a dictionary to help you.', lead_in = 'B
Vocabulary
...
English.' where task_number = 21 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'In the following sentences, there is a vocabulary error of some kind.  Identify and describe the problem.
...
5. Every day I like to go footing after work.  It’s good exercise and it helps me to relax.', lead_in = 'While you might have a reasonably good understanding of words in English, using a dictionary can provide you
...
words have more than one meaning and sometimes defining exactly what a word means can be complex.' where task_number = 22 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'In the following sentences, you can find the following examples of collocation:
...
6. She was caught in a vicious circle.', lead_in = 'When thinking about the meaning of words in English, we often need to consider different aspects of meaning.
...
talking about people.  This is known as collocation.' where task_number = 23 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Match the terms 1-3 with the definitions A – C.
...
3. intonation
C. an individual sound', lead_in = 'To summarise, in order to be able to say we know the meaning of a word, we need to think about the following:
•
semantic meaning
•
register
...
includes not only individual sounds, but also different rhythms and the way our voices rise and fall.' where task_number = 24 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Match the problems 1-3 with the outcomes A-C.
...
C. the speaker can sound arrogant and demanding', lead_in = 'It is important to focus on phonology in the classroom because, if incorrect, it can lead to breakdowns in
communication between two speakers conversing in English.' where task_number = 25 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'What are these words?', lead_in = 'Individual sounds
...
note that words written in phonemic script are written between slanting lines):' where task_number = 26 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Where does the stress fall on the following words?
...
10. articulate (adjective)', lead_in = 'Word Stress

All words of two or more syllables in English have word stress.  The stress can be marked by putting a box (or
circle) over the vowel sound in the stressed syllable.' where task_number = 27 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = '1. Mark the main stress on the words in the following ''word family''.  Why do you think non-native speakers
...
to import / an import', lead_in = 'There are few hard and fast rules that govern word stress.  One rule that does work is that words ending in –ion
(e.g. station, election, division etc.) have stress on the syllable immediately before the –ion syllable. This is the
case even for quite long words:' where task_number = 28 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Underline the /ə/ in the following words:
...
The first syllable is stressed.  The second syllable is unstressed and contains the weak sound.' where task_number = 29 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Think of all the texts you have read so far today and comment on the way in which you have read these texts.', lead_in = 'Section 3: Language Skills: Reading, Listening,
...
understood and interpreted it correctly.' where task_number = 30 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Below are some different text types.  Think about which of the above reading sub-skills we would use to read
these texts.
...
4. An advertisement for a job that might be suitable.', lead_in = 'Typically, native speaker readers deploy different reading sub-skills when they approach texts.  We do not do
...
Reading to infer  when we read to understand a writer’s implicit message in a text' where task_number = 31 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = '•
What problems are there with this way of reading?', lead_in = 'When people read in a language that is not their first language, they often forget to use appropriate language
skills and will read all texts in a great deal of detail.  They are also likely to overuse bi-lingual dictionaries,
pausing in their reading every time they find a word they do not understand.' where task_number = 32 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = '•
Make a list of reasons why you think non-native speakers might find listening more challenging than
reading.', lead_in = 'B
Listening
...
from a conversation overheard to a radio news broadcast.)' where task_number = 33 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = '•
Think of three different situations in which you listened today.  Make a note of who you were listening to,
your motivation for listening and describe how you listened.', lead_in = 'As is the case with reading, we do not always listen to everything in the same way.  The way in which we listen is
...
more carefully (unless you feel it would be socially inappropriate to do so).' where task_number = 34 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Match the above listening sub-skills to the following listening texts.
...
6. A radio programme on a topic that is mildly interesting for you.', lead_in = 'All of the above suggests that listening is similar to reading in the way that there are different sub-skills that we
...
Intensive listening
•
Listening to infer meaning' where task_number = 35 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = '•
Think about this disparity between 2 years’ study of a language and a lack of ability to speak.  Why do you
think this happens?', lead_in = 'C
Speaking
...
and that they are not very ‘fluent’ in English.' where task_number = 36 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Look at the following examples of learner language and decide whether the student has managed to
...
B: I stay here 5 weeks.', lead_in = 'While grammar is important, students need to learn how to activate their passive grammatical knowledge by
...
effectively.' where task_number = 37 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Decide if the following descriptions of conversations are transactional or interactional.
...
6. You go out with your boss and other colleagues for a drink after work.', lead_in = 'Most oral interaction can usually be described as transactional or interactional.  Transactional communication
...
chat with a friend or make small talk with a colleague.' where task_number = 38 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = '•
Make a list of reasons why you think speaking fluency practice could help learners’ language development.', lead_in = 'Some learners are unsure of the value of doing speaking fluency activities in the classroom.  They feel that the
...
learners in having speaking fluency practice.' where task_number = 39 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Which of the following characteristics would you associate with spoken language (S) and which with written
language (W).
...
12. Involves negotiation of meaning between the communicator and the person they are communicating with.', lead_in = 'D
Writing
...
(spoken language).' where task_number = 40 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Look at the learner errors in the sentences below.  Identify the mistakes and try to decide why the learner made
these mistakes.

1. She through the ball hard so it hurt when I court it.
2. My brther livs in Swedn.
3. However, hard I try it never works.
4. first of all he invited me to sit down after that he offered me a coffee I was very surprised by his politeness', lead_in = 'Writing also involves many micro-skills such as correct letter formation (orthography), spelling and punctuation.' where task_number = 41 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = '1. Look at the samples of learners’ writing below and identify the difficulties encountered by these learners
and other learners whose first language do not have a Roman script.

2. Suggest some activities that teachers can use to help students to develop basic writing skills.', lead_in = 'These skills can be particularly challenging for learners whose first language does not have a Roman script, for
example, Arabic and Chinese speakers.' where task_number = 42 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Below are some extracts from lesson plans.  Decide which part of the lesson plan (1-7 above) they may come
from.
...
f. To get students interested in the topic of the listening text.', lead_in = 'Section 4: Planning and Resources
...
some area of your teaching you want to improve in this
lesson
4.
 Stage
...
what students/teacher are doing at any particular stage of
the lesson
6.
 Interaction pattern

the direction of the communication (teacher to
students/students to students)
7.
 Stage aim

the reason for doing this stage of the lesson and how it
relates to the overall aim of the lesson' where task_number = 43 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Resource
Can be used for

1. Published coursebook
a. Encouraging students to expand their vocabulary and to find out
about new words on their own

2. Cassette or CD player
b. Developing students’ ability to read real texts

3. Newspapers in English
c. Giving students work which can be tailored to their individual
needs
...
e. Finding information on a particular topic area and developing
reading skills

6. Whiteboard
f. Developing students’ language and skills in a structured way and
allowing them to review at home

7. TV/radio
g. Developing students’ listening skills with specially prepared or real
materials

8. Teacher’s own materials
h. Writing down new words for students to focus on, making the
form, meaning or pronunciation features of a language area clear

9. Dictionaries
i.
Showing pre-prepared work on a large screen for clarity', lead_in = 'B
Resources

In your teaching situation you will have some resources available to you.  The range will vary and depend on
where you are teaching.  Below is a list of possible resources you may be able to use.  Match them to their uses.' where task_number = 44 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Below are some instructions given by teachers.  In each case the group of learners had difficulty following the
...
4. Look at the question at the bottom of the page and think about an answer.', lead_in = 'Section 5: Developing Teaching Skills and
Professionalism
...
to manage the classroom, mainly in the form of instructions.' where task_number = 45 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Imagine you are the teacher of the students who made the comments below.  How would
you respond to them?
...
4. Could we just talk in class and not use any books?', lead_in = 'Managing the learning environment
...
activities that learners do in pairs or small groups.  You also use pair/group work to promote peer teaching,
...
one-to-one translation with the first language.  Also, in a multilingual classroom it would be impossible to expect
the teacher to know all the languages represented!' where task_number = 46 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Below is a description of an activity you might use in a language learning classroom.
...
been given.  The students have 20 minutes to ask the assistants about their courses and agree a transaction.', lead_in = 'Organisation of the classroom
...
board, you may need to ensure that this learner is seated near the board at all times.' where task_number = 47 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'You notice that your students have been confusing certain areas of language.  You decide to have a lesson
...
10. Live (verb) / live (something happening now)', lead_in = 'Dealing with language
...
stress visually by highlighting what is stressed.' where task_number = 48 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Below are descriptions of two activities used by teachers.  In 1 the teacher was trying to develop students’
...
write something about it for homework.', lead_in = 'B
Developing skills
...
they must know beforehand what information they are looking for.' where task_number = 49 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
update public.pre_course_task_items set prompt = 'Consider how the following form part of a teacher’s professionalism:
...
• ESOL/EFL and teacher training research and development world-wide', lead_in = 'C
Professionalism
...
you use to a dress code!  You will also be responsible for ensuring that you uphold the school/college’s equal
...
more experience.  These include ICELT (in-Service Certificate in English Language Teaching ) and  Delta (Diploma
in English Language Teaching to Adults), also offered by Cambridge English.' where task_number = 50 and section_id in (select id from public.pre_course_task_sections where source = 'cambridge');
