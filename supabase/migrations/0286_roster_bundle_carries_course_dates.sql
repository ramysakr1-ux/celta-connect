-- The roster bundle was withholding the course's own dates.
--
-- hub_roster_bundle returns everything the trainer hub's roster needs in one
-- round trip (migration 0273). Its `course` object carried total_hours and
-- center_id and nothing else, which was fine until attendance stopped being
-- measured against the whole course and started being measured against the
-- hours actually taught so far -- which needs start_date and end_date.
--
-- The TypeScript widened; the SQL did not, so course.start_date came back
-- undefined, the calculation fell back to the course total, and every
-- candidate on the demo still read 75% and stayed flagged "Attendance below
-- 80%" after the fix was deployed. A bundle is a second definition of a query
-- and it has to be changed in step with the first.
--
-- Same function, two more columns.
