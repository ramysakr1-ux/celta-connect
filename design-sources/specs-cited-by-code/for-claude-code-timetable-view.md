...
- Content: title (11.5px, weight varies by category), optional meta line (10px, muted), optional camera-link icon, optional "You teach" tag (ink-colored, only visible in "Mine" mode -- not gold, which is reserved for brand + above-standard grade).
...
- The "You teach" tag only appears in "Mine" mode, on the viewer's own personal TP slot.
...
## Motion (added 15 Sep 2026)

Cards are read-only but must feel live under the cursor, not pasted on.

- **Hover lift** on every populated card: `transform: translateY(-3px) scale(1.025)`, shadow deepens to `0 14px 30px oklch(23.5% 0.017 65 / 0.14), 0 2px 6px oklch(23.5% 0.017 65 / 0.06)`, crystal border brightens to `oklch(100% 0 0 / 0.95)`, card raised above neighbours (`z-index: 2; position: relative`). Transition 220ms `cubic-bezier(.2,.8,.2,1)` on transform, 220ms ease on shadow/border. Cursor stays `default` — the lift signals presence, not draggability.
- **Join icon hover**: `scale(1.15)` plus a 4px teal halo ring `oklch(38% 0.072 195 / 0.15)`, 160ms. Only when the icon is in its clickable state.
- **Entrance ripple** on load and on week change: each cell fades in from `opacity 0, translateY(6px)` over 420ms with the same easing, delayed by `column × 28ms + row × 40ms` so the grid paints diagonally from top-left.
- Cell wrappers use `overflow: visible` so the lifted card is not clipped.
- Tile size never changes on hover beyond the 2.5% scale; all tiles remain identical in size.
